...
class TmapServices {
  ...
  static Future<Route> getRoute(LatLng origin, LatLng destination,
    ...
    if (passList != null) {
      ...
      for (LatLng iter in passList) // 경유지 정보 추가. 최대 5곳 가능
        body +=
            (iter.longitude.toString() + ',' + iter.latitude.toString() + '_');
    ...
    try {
      ...
      if (passList != null)
        for (int points = 0; points < result.locations.length; points++) {
          var lat =
              passList[0].latitude - result.locations[points].location.latitude;
          var lng = passList[0].longitude -
              result.locations[points].location.longitude;
  ...
  static Future<String> reverseGeocoding(LatLng location) async {
    http.Response response = await http.get(
        "https://apis.openapi.sk.com/tmap/geo/reversegeocoding?version=1&lat=${location.latitude}&lon=${location.longitude}&addressType=A03&appKey=$projectKey");
  ...
  static Future<List<LatLng>> getNearRoadInformation(LatLng position) async {
    try {
      http.Response response = await http.get(
          "https://apis.openapi.sk.com/tmap/road/nearToRoad?version=1&appKey=$projectKey&lat=${position.latitude}&lon=${position.longitude}");
      Map values = jsonDecode(response.body);
      List<LatLng> result = [];
